...
namespace {
fz::Stage* Cdf97_fromHeader(const uint8_t* config, size_t config_size) {
    using fz::DataType; using fz::Cdf97Config; using fz::Cdf97Stage;
    // Element type (FLOAT32/FLOAT64) is stored in the config's first byte.
    DataType dt = (config_size > 0) ? static_cast<DataType>(config[0]) : DataType::FLOAT64;
    fz::Stage* stage = (dt == DataType::FLOAT32) ? static_cast<fz::Stage*>(new Cdf97Stage<float>())
                                                  : static_cast<fz::Stage*>(new Cdf97Stage<double>());
    if (config_size > 0) stage->deserializeHeader(config, config_size);
    return stage;
}
}  // namespace
FZ_REGISTER_STAGE_FACTORY(fz::StageType::CDF97, Cdf97_fromHeader);
