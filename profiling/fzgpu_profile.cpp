/**
 * fzgpu_profile — FZ-GPU pipeline benchmark
 *
 * Pipeline (from examples/presets/fzgpu.toml):
 *   LorenzoQuant(NOA, zigzag, r=512, cap=20%) → BitplaneRZE
 *
 * BitplaneRZE fuses the 32×32 bitplane transpose and zero-group elimination
 * into a single shared-memory kernel — no separate Bitshuffle stage.  This is
 * FZ-GPU's actual lossless back-end.  Expected to be the fastest compress/
 * decompress pipeline in the library for smooth scientific fields.
 *
 * Phases:
 *   Phase A — PREALLOCATE: cold + N steady-state compress and decompress
 *   Phase B — CUDA Graph:  attempted (BitplaneRZE performs D2H reads during both
 *             compress and decompress, so graph capture is expected to fail gracefully)
 *
 * Usage:
 *   ./fzgmod-profile-fzgpu [runs] [eb]
 ...
 *        -o fzgpu ./build_profiling/bin/profiling/fzgmod-profile-fzgpu
 *   nsys-ui fzgpu.nsys-rep
 */
...
// Returns the LorenzoQuant stage pointer so the caller can inject value_base.
static LorenzoQuantStage<float, uint16_t>* build_fzgpu(Pipeline& p, float eb,
...
int main(int argc, char* argv[]) {
    ...
    print_header("FZ-GPU profiling benchmark — CLDHGH " + std::to_string(DIM_X) + "x" + std::to_string(DIM_Y));
    std::cout << "  Pipeline : LorenzoQuant(NOA,zigzag,r=512,cap=20%) -> BitplaneRZE\n"
              << "  Dataset  : " << DATA_PATH << "\n"
    ...
    // =========================================================================
    //  Phase B — CUDA Graph (compress)
    //  BitplaneRZE performs D2H reads during compress, so graph capture is
    //  expected to fail.  The try/catch reports the reason and continues.
    // =========================================================================
    print_header("Phase B — CUDA Graph compress (expected to fail for BitplaneRZE)");

    try {
        Pipeline gp(data_bytes, MemoryStrategy::PREALLOCATE, POOL_MULT);
        auto* glq = build_fzgpu(gp, eb, DIM_X, DIM_Y);
        glq->setValueBase(ld.data_range);   // skip NOA scan during capture
