...
template <typename E>
int high_level<E>::build_book(phf::Buf<E>* buf, uint32_t* h_hist, uint16_t const rt_bklen, HF_STREAM stream)
{
  buf->register_runtime_bklen(rt_bklen);
  float _time_book = 0.0;

  phf_CPU_build_canonized_codebook_v2<E, H4>(
      h_hist, rt_bklen, buf->h_bk4.get(), buf->h_revbk4.get(), &_time_book);
  ...
  cudaMemcpyAsync(
      buf->d_bk4.get(), buf->h_bk4.get(), rt_bklen * sizeof(H4), cudaMemcpyHostToDevice, (cudaStream_t)stream);
  cudaMemcpyAsync(
      buf->d_revbk4.get(), buf->h_revbk4.get(), buf->revbk4_bytes, cudaMemcpyHostToDevice, (cudaStream_t)stream);
