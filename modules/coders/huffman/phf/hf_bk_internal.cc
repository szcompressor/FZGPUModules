/**
 * Adapted from PHF reference (origin/v1.1.0_dev:modules/codec/huffman/hf_bk_internal.seq.cc)
 * Original authors: Jiannan Tian (Indiana University, Argonne National Laboratory)
 * Changes:
 *   - Removed #include "timer.hh" (included in reference but not used in function bodies).
 */
...
void NODE_STACK::push(NODE_STACK* s, NodeType* n, T path, T len)
{
    if (s->depth + 1 <= NODE_STACK::MAX_DEPTH) {
        ...
        throw std::runtime_error(
            "[fz::err::hf::traverse_stack]: exceeding MAX_DEPTH, stack overflow.");
    }
}
...
void NODE_STACK::inorder_traverse(NodeType* root, H* book)
{
    ...
    while (!done && p != nullptr) {
        if (p->left || p->right) {
            ...
            len += 1;
            if (len > MAX_LEN)
                __PSZDBG__FATAL("exceeding max len: " + std::to_string(MAX_LEN));
        }
        else {
