...
#include "fused/lorenzo_quant/lorenzo_quant.h"  // ErrorBoundMode, resolveApproxRelMode — FusedQuantAdaptiveLorenzoStage
...
/// Inverse: replay each tile's recorded variant. `d_offsets` is the exclusive
/// scan of the centering flags (rebuilt by launchAdaptiveLorenzoFlags), used to
/// index the compacted means.
template<typename T>
...
/// Compact the dense per-tile means to only the centering tiles and pack the
/// mode bytes 4-to-1.
template<typename T>
...
/// Rebuild the centering flags from the packed modes (inverse-side, so the same
/// exclusive scan reproduces the compaction offsets).
