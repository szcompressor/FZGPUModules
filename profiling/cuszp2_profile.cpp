/**
 * cuszp2_profile — cuSZp2 optimization benchmark
 *
 * Pipeline (from examples/presets/cuszp2.toml):
 *   Quantizer(linear, ABS) → Lorenzo(block=32) → AdaptiveBitpack(block=32, outlier)
 *
...
// ── Build cuSZp2 pipeline (no finalize — caller decides graph mode first) ──────
// Matches examples/presets/cuszp2.toml exactly.
static void build_cuszp2(Pipeline& p, float eb, size_t dim_x, size_t dim_y) {
    p.setDims(dim_x, dim_y, 1);  // must precede addStage<LorenzoStage>
    ...
    ab->setBlockSize(32);
    ab->setOutlierSelection(true);  // cuSZp2: per-block outlier-aware packing
    p.connect(ab, lrz);
}
...
int main(int argc, char* argv[]) {
    ...
    print_header("cuSZp2 profiling benchmark — CLDHGH " + std::to_string(DIM_X) + "x" + std::to_string(DIM_Y));
    std::cout << "  Pipeline : Quantizer(linear,ABS) -> Lorenzo(32) -> AdaptiveBitpack(32,outlier)\n"
              << "  Dataset  : " << DATA_PATH << "\n"
    ...
    try {
        ...
        build_cuszp2(gp, eb, DIM_X, DIM_Y);
