...
template <typename T = float, typename E = uint32_t, int LINEAR_BLOCK_SIZE = DEFAULT_LINEAR_BLOCK_SIZE>
    ...
    int     radius)
{
...

    {
       ...
       // if(TIX==0 and BIX==0 and BIY==0 and BIZ==0)
        ...
        fz::device_api::spline3d_layout2_interpolate<T, T, FP,LINEAR_BLOCK_SIZE, SPLINE3_COMPR, false>(
...
template <
    ...
    typename FP,
    int LINEAR_BLOCK_SIZE>
__global__ void fz::x_spline3d_infprecis_32x8x8data(
    EITER   ectrl,        // input 1
    DIM3    ectrl_size,   //
    STRIDE3 ectrl_leap,   //
    TITER   anchor,       // input 2
    ...
    int     radius)
{
    ...
    fz::device_api::spline3d_layout2_interpolate<T, T, FP, LINEAR_BLOCK_SIZE, SPLINE3_DECOMPR, false>(
        shmem.data, shmem.ectrl, data_size, eb_r, ebx2, radius);
    //if(TIX==0 and BIX==0 and BIY==0 and BIZ==0)
    //        printf("dsz: %d %d %d\n",data_size.x,data_size.y,data_size.z);
