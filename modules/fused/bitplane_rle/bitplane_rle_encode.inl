// Fused bitplane-transpose + zero-byte run-length ENCODE kernel.
//
// Prior work: this kernel is the FZ-GPU lossless encoder. It is adapted from
// `KERNEL_CUHIP_fz_fused_encode` in the cuSZ/FZ-GPU codebase.
//   Original authors: Boyuan Zhang (kernel), Jiannan Tian (refactor).
//   FZ-GPU: Boyuan Zhang et al., "FZ-GPU: A Fast and High-Ratio Lossy
//   Compressor for Scientific Computing Applications on GPUs", HPDC '23.
// Only the namespace and surrounding build dependencies were changed; the
// kernel body is preserved. See THIRD_PARTY.md.
