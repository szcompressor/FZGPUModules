...
static constexpr int RZE_TEMP = 4096;   // shared bytes for prefix-sum scratch + bitmap levels
                                         ...
                                         // RZEStage::onFinalize / execute() for the supported set)

using fz::lc_detail::byte;

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Encode kernel — one block per chunk.  Runs d_RZE<T,CS> on the chunk in shared
    ...
    uint32_t total_in_bytes)
{
    ...
    for (int i = threadIdx.x; i < CS; i += RZE_TPB)
        s_in[i] = (i < in_size) ? d_in[in_off + i] : (byte)0;
    __syncthreads();

    int  csize = in_size;
    bool good  = lc_detail::d_RZE<T, CS>(csize, s_in, s_out, s_temp);
    ...
    const uint32_t* __restrict__ d_orig_sizes)
{
    ...
    lc_detail::d_iRZE<T, CS>(csize, s_in, s_out, s_temp);  // sets csize = orig_size
    __syncthreads();
...
// T. Supported chunk sizes: 4096, 8192, 16384 (see RZEStage::execute()).
...
{
#define FZ_RZE_ENCODE_CASE(CS_VAL)                                                                          \
    case CS_VAL:                                                                                             \
        switch (word_size) {                                                                                 \
            case 1: rzeEncodeKernel<uint8_t,  CS_VAL><<<n_chunks, RZE_TPB, 0, stream>>>(d_in, d_scratch, d_sizes, in_bytes); break; \
            case 2: rzeEncodeKernel<uint16_t, CS_VAL><<<n_chunks, RZE_TPB, 0, stream>>>(d_in, d_scratch, d_sizes, in_bytes); break; \
            case 4: rzeEncodeKernel<uint32_t, CS_VAL><<<n_chunks, RZE_TPB, 0, stream>>>(d_in, d_scratch, d_sizes, in_bytes); break; \
            case 8: rzeEncodeKernel<uint64_t, CS_VAL><<<n_chunks, RZE_TPB, 0, stream>>>(d_in, d_scratch, d_sizes, in_bytes); break; \
            default: throw std::runtime_error("RZEStage: word_size must be 1, 2, 4, or 8");                  \
        }                                                                                                     \
        break;
    switch (chunk_size) {
        FZ_RZE_ENCODE_CASE(4096)
        FZ_RZE_ENCODE_CASE(8192)
        FZ_RZE_ENCODE_CASE(16384)
        default: throw std::runtime_error("RZEStage: chunk_size must be 4096, 8192, or 16384");
    }
#undef FZ_RZE_ENCODE_CASE
}
...
{
#define FZ_RZE_DECODE_CASE(CS_VAL)                                                                          \
    case CS_VAL:                                                                                             \
        switch (word_size) {                                                                                 \
            case 1: rzeDecodeKernel<uint8_t,  CS_VAL><<<n_chunks, RZE_TPB, 0, stream>>>(d_in, d_out, in_off, comp_sz, out_off, orig_sz); break; \
            case 2: rzeDecodeKernel<uint16_t, CS_VAL><<<n_chunks, RZE_TPB, 0, stream>>>(d_in, d_out, in_off, comp_sz, out_off, orig_sz); break; \
            case 4: rzeDecodeKernel<uint32_t, CS_VAL><<<n_chunks, RZE_TPB, 0, stream>>>(d_in, d_out, in_off, comp_sz, out_off, orig_sz); break; \
            case 8: rzeDecodeKernel<uint64_t, CS_VAL><<<n_chunks, RZE_TPB, 0, stream>>>(d_in, d_out, in_off, comp_sz, out_off, orig_sz); break; \
            default: throw std::runtime_error("RZEStage: word_size must be 1, 2, 4, or 8");                  \
        }                                                                                                     \
        break;
    switch (chunk_size) {
        FZ_RZE_DECODE_CASE(4096)
        FZ_RZE_DECODE_CASE(8192)
        FZ_RZE_DECODE_CASE(16384)
        default: throw std::runtime_error("RZEStage: chunk_size must be 4096, 8192, or 16384");
    }
#undef FZ_RZE_DECODE_CASE
}
    ...
    const std::vector<size_t>& sizes)
{
    ...
    if (chunk_size_ != 4096 && chunk_size_ != 8192 && chunk_size_ != 16384)
        throw std::runtime_error(
            "RZEStage: chunk_size must be 4096, 8192, or 16384; got "
