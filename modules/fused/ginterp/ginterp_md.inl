 ...
 * but unused in the MVP host wrappers — they are wired up in phase 2 when
 * `setAutoTuning(uint8_t > 0)` is added.
 */

#ifndef FZGM_KERNEL_GINTERP_MD_INL
#define FZGM_KERNEL_GINTERP_MD_INL
...
__device__ void fz::ginterp::device_api::spline_layout_interpolate(
    volatile T1 s_data[AnchorBlockSizeZ * numAnchorBlockZ + (SPLINE_DIM >= 3)]
                       [AnchorBlockSizeY * numAnchorBlockY + (SPLINE_DIM >= 2)]
                       [AnchorBlockSizeX * numAnchorBlockX + (SPLINE_DIM >= 1)],
    volatile T2 s_ectrl[AnchorBlockSizeZ * numAnchorBlockZ + (SPLINE_DIM >= 3)]
...
int PROFILE_NUM_BLOCK_Z = 4, 
int LINEAR_BLOCK_SIZE = DEFAULT_LINEAR_BLOCK_SIZE>
__global__ void fz::ginterp::c_spline_profiling_data(
    TITER   data,
    DIM3    data_size,
    STRIDE3 data_leap,
    TITER errors)
...
__global__ void fz::ginterp::c_spline_profiling_data_2(
    TITER   data,
    DIM3    data_size,
    STRIDE3 data_leap,
    TITER errors)
{
...

    {
        ...
        fz::ginterp::device_api::auto_tuning_2<T, SPLINE_DIM, PROFILE_NUM_BLOCK_X, PROFILE_NUM_BLOCK_Y, PROFILE_NUM_BLOCK_Z, LINEAR_BLOCK_SIZE>(
...
__global__ void fz::ginterp::c_spline_infprecis_data(
    TITER   data,
    DIM3    data_size,
    STRIDE3 data_leap,
    EITER   ectrl,
...
template <
    ...
    int numAnchorBlockZ,  // Number of Anchor blocks along Z
    int LINEAR_BLOCK_SIZE>
__global__ void fz::ginterp::x_spline_infprecis_data(
    EITER   ectrl,        // input 1
    DIM3    ectrl_size,   //
    STRIDE3 ectrl_leap,   //
    TITER   anchor,       // input 2
    ...
    INTERPOLATION_PARAMS intp_param)
{
    ...
    fz::ginterp::device_api::spline_layout_interpolate<T, T, FP, LEVEL, SPLINE_DIM, AnchorBlockSizeX, AnchorBlockSizeY, AnchorBlockSizeZ, numAnchorBlockX, numAnchorBlockY, numAnchorBlockZ, LINEAR_BLOCK_SIZE, SPLINE3_DECOMPR, false>(
        shmem_data, shmem_ectrl, data_size, eb_r, ebx2, radius, intp_param);
...
template <typename TITER>
__global__ void fz::ginterp::reset_errors(TITER errors)
{
    if(TIX<36)
        errors[TIX]=0;
}
...
__device__ void fz::ginterp::device_api::spline_layout_interpolate_att(
    volatile T s_data[AnchorBlockSizeZ * numAnchorBlockZ + (SPLINE_DIM >= 3)]
    [AnchorBlockSizeY * numAnchorBlockY + (SPLINE_DIM >= 2)]
    [AnchorBlockSizeX * numAnchorBlockX + (SPLINE_DIM >= 1)],
    DIM3    data_size,
...
__global__ void fz::ginterp::pa_spline_infprecis_data(
    TITER   data,
    DIM3    data_size,
    STRIDE3 data_leap,
    DIM3 sample_starts,
