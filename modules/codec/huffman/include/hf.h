...
// codec
phf_codec* capi_phf_create(size_t const inlen, phf_dtype const t,
                           int const bklen);
...
int capi_phf_encode(phf_codec* codec, void* in, size_t const inlen,
                    uint8_t** encoded, size_t* enc_bytes, phf_stream_t);
int capi_phf_decode(phf_codec* codec, uint8_t* encoded, void* decoded,
                    phf_stream_t);

// helpers
size_t capi_phf_reverse_book_bytes(uint16_t bklen, size_t BK_UNIT_BYTES,
                                   size_t SYM_BYTES);
uint8_t* capi_phf_allocate_reverse_book(uint16_t bklen, size_t BK_UNIT_BYTES,
                                        size_t SYM_BYTES);

void pszanalysis_hf_buildtree(uint32_t* freq, int const bklen, double* entropy,
                              double* cr, int const symbol_byte);
