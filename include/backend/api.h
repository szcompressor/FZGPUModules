...
// NVRTC compiles device code only and already provides the device-side
// intrinsics (atomics, shuffles, __trap, math). Pulling in the host runtime
// header <cuda_runtime.h> here would fail — NVRTC does not ship it. The fused
// kernels generated by the NVRTC codegen path (src/pipeline/nvrtc_fusion.*)
// include this header transitively via the vendored LC device codecs, so this
// branch lets those device-only translation units compile with no host runtime.
