/**
 * speed_profile — Speed preset benchmark
 *
 * Pipeline (from examples/presets/speed.toml):
 *   Quantizer(REL, zigzag, r=32768) → Bitshuffle(ew=4, block=16384) → RZE(w=1)
 *
 * The throughput ceiling preset: fewest stages, no predictor.  Useful for
 * isolating the minimum overhead the framework adds on top of pure kernel cost
 * and for establishing a GB/s upper bound that heavier pipelines approach.
 *
 * Phases:
 *   Phase A — PREALLOCATE: cold + N steady-state compress and decompress
 *   Phase B — CUDA Graph:  capture + N replay compress (REL value_base injected)
 *
 * Usage:
 *   ./fzgmod-profile-speed [runs] [eb]
 *   runs : integer > 0       (default: 10)
 *   eb   : positive float    (default: 1e-3, interpreted as REL)
 *
 * Nsys:
 *   nsys profile --trace=cuda,nvtx --capture-range=cudaProfilerApi \
 *        -o speed ./build_profiling/bin/profiling/fzgmod-profile-speed
 *   nsys-ui speed.nsys-rep
 */
...
static QuantizerStage<float, uint32_t>* build_speed(Pipeline& p, float eb) {
    auto* quant = p.addStage<QuantizerStage<float, uint32_t>>();
    quant->setErrorBound(eb);
    quant->setErrorBoundMode(ErrorBoundMode::REL);
...
int main(int argc, char* argv[]) {
    ...
    print_header("Speed preset profiling benchmark — CLDHGH " + std::to_string(DIM_X) + "x" + std::to_string(DIM_Y));
    std::cout << "  Pipeline : Quantizer(REL,zigzag,r=32768) -> Bitshuffle(ew=4) -> RZE(w=1)\n"
              << "  Dataset  : " << DATA_PATH << "\n"
              << "  EB       : " << std::scientific << std::setprecision(1) << eb << " (REL)\n"
    ...
    std::cout << "  Loaded " << std::fixed << std::setprecision(2)
              ...
              << "  Value range: " << std::fixed << std::setprecision(4) << ld.data_range
              << "  max_abs: " << ld.max_abs << "  (REL value_base for graph capture)\n\n";
    ...
    //  REL mode: inject max_abs as value_base so execute() skips the D2H scan.
    // =========================================================================
    print_header("Phase B — CUDA Graph compress");

    try {
        Pipeline gp(data_bytes, MemoryStrategy::PREALLOCATE, POOL_MULT);
        auto* gquant = build_speed(gp, eb);
        gquant->setValueBase(ld.max_abs);   // skip REL scan during capture
    ...
    if (d_recon && d_recon_sz == data_bytes) {
        ...
        std::cout << std::fixed
                  ...
                  << stats.max_error << "  (abs bound=" << abs_bound << ")  "
