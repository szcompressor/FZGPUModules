...
class hf_canon_reference {
 ...
 public:
  ...
  hf_canon_reference(uint16_t booklen)
    : booklen(booklen), _icb(nullptr), _ocb(nullptr), _canon(nullptr),
      _numl(nullptr), _iterby(nullptr), _first(nullptr), _entry(nullptr), _keys(nullptr)
  {
    init();
  }
  hf_canon_reference(const hf_canon_reference& other); // Copy constructor
  hf_canon_reference& operator=(const hf_canon_reference& other); // Copy assignment
  ~hf_canon_reference() {
    // delete[] _icb;
    delete[] _ocb;
    delete[] _canon;
...
class modules {
 ...
 public:
  static void GPU_coarse_encode_phase1(
      E* in_data, const size_t data_len, H* in_book, const uint32_t book_len, const int numSMs,
  ...
  static void GPU_fine_encode_phase1_2(
      E* in, const size_t len, H* book, const uint32_t bklen, H* bitstream, M* par_nbit, M* par_ncell,
      const uint32_t nblock, E* brval, uint32_t* bridx, uint32_t* brnum, void* stream);
  ...
  static void GPU_coarse_encode(
      E* in_data, size_t data_len, H* in_book, uint32_t book_len, int numSMs, phf::par_config hfpar,
      // internal buffers
  ...
  static void GPU_fine_encode(
      E* in_data, size_t data_len, H* in_book, uint32_t book_len, phf::par_config hfpar,
      // internal buffers
      H* d_scratch4, M* d_par_nbit, M* h_par_nbit, M* d_par_ncell, M* h_par_ncell, M* d_par_entry,
      M* h_par_entry, H* d_bitstream4, size_t bitstream_max_len, E* d_brval, uint32_t* d_bridx,
      uint32_t* d_brnum,
      // output
      size_t* out_total_nbit, size_t* out_total_ncell, void* stream);
  ...
  static void GPU_scatter(E* val, uint32_t* idx, const uint32_t h_num, E* out, void* stream);
};
