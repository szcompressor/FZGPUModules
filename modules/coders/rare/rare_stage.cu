 ...
 * `d_RARE<T>` / `d_iRARE<T>` device functions on it. Inter-chunk packing offsets
 * are computed with a CUB exclusive scan, identical to RREStage — the host-side
 * container format doesn't know or care about RARE's extra per-chunk `keep`
 * byte, since that lives inside the chunk's own compressed bytes (accounted
 * for by `csize`), not in the container.
 *
 * Algorithm reference: d_RARE.h from the LC framework (Burtscher et al.,
...
static constexpr int RARE_TPB  = 512;    // threads per block (must match lc_chunk_components.cuh TPB)
static constexpr int RARE_TEMP = 4096;   // shared bytes for histogram/prefix-sum scratch + bitmap levels
                                          // (sized for the largest supported CS = 16384; see
                                          // RAREStage::execute() for the supported set)

using fz::lc_detail::byte;

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Encode kernel — one block per chunk. Runs d_RARE<T,CS> on the chunk in shared
// memory; falls back to verbatim storage (high-bit flag) when RARE fails or
// fails to shrink the chunk. CS is compile-time; see launchEncode for the
// supported instantiations.
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
template <typename T, int CS>
static __global__ void __launch_bounds__(RARE_TPB)
rareEncodeKernel(
    ...
    uint32_t total_in_bytes)
{
    ...
    __shared__ __align__(16) byte s_temp[RARE_TEMP];
    ...
    for (int i = threadIdx.x; i < CS; i += RARE_TPB)
        s_in[i] = (i < in_size) ? d_in[in_off + i] : (byte)0;
    __syncthreads();

    int  csize = in_size;
    bool good  = lc_detail::d_RARE<T, CS>(csize, s_in, s_out, s_temp);
    // Mirrors RREStage: d_RARE writes its final bitmap level (and, on the
    ...
    if (good && csize < in_size) {
        for (int i = threadIdx.x; i < csize; i += RARE_TPB)
            out[i] = s_out[i];
        if (threadIdx.x == 0)
            d_sizes[cid] = (uint32_t)csize;
    } else {
        // Incompressible: store the original chunk verbatim (s_in is untouched
        // by d_RARE, which only writes s_out / s_temp).
        for (int i = threadIdx.x; i < in_size; i += RARE_TPB)
            out[i] = s_in[i];
        if (threadIdx.x == 0)
    ...
    const uint32_t* __restrict__ d_orig_sizes)
{
    ...
    for (int i = threadIdx.x; i < (int)comp_size; i += RARE_TPB)
        s_in[i] = in[i];
    __syncthreads();

    int csize = (int)comp_size;
    lc_detail::d_iRARE<T, CS>(csize, s_in, s_out, s_temp);  // sets csize = orig_size
    __syncthreads();

    for (int i = threadIdx.x; i < (int)orig_size; i += RARE_TPB)
        out[i] = s_out[i];
}

// ━━━━ helper kernels (word-size independent; mirror RRE/RZEStage) ━━━━━━━━━
static __global__ void rareStripFlagKernel(
...
static __global__ void rareAddOffsetKernel(
...
static __global__ void rarePackKernel(
...
// T. Supported chunk sizes: 4096, 8192, 16384 (see RAREStage::execute()).
...
{
#define FZ_RARE_ENCODE_CASE(CS_VAL)                                                                          \
    case CS_VAL:                                                                                             \
        switch (word_size) {                                                                                 \
            case 1: rareEncodeKernel<uint8_t,  CS_VAL><<<n_chunks, RARE_TPB, 0, stream>>>(d_in, d_scratch, d_sizes, in_bytes); break; \
            case 2: rareEncodeKernel<uint16_t, CS_VAL><<<n_chunks, RARE_TPB, 0, stream>>>(d_in, d_scratch, d_sizes, in_bytes); break; \
            case 4: rareEncodeKernel<uint32_t, CS_VAL><<<n_chunks, RARE_TPB, 0, stream>>>(d_in, d_scratch, d_sizes, in_bytes); break; \
            case 8: rareEncodeKernel<uint64_t, CS_VAL><<<n_chunks, RARE_TPB, 0, stream>>>(d_in, d_scratch, d_sizes, in_bytes); break; \
            default: throw std::runtime_error("RAREStage: word_size must be 1, 2, 4, or 8");                 \
        }                                                                                                     \
        break;
    switch (chunk_size) {
        FZ_RARE_ENCODE_CASE(4096)
        FZ_RARE_ENCODE_CASE(8192)
        FZ_RARE_ENCODE_CASE(16384)
        default: throw std::runtime_error("RAREStage: chunk_size must be 4096, 8192, or 16384");
    }
#undef FZ_RARE_ENCODE_CASE
}
...
{
#define FZ_RARE_DECODE_CASE(CS_VAL)                                                                          \
    case CS_VAL:                                                                                             \
        switch (word_size) {                                                                                 \
            case 1: rareDecodeKernel<uint8_t,  CS_VAL><<<n_chunks, RARE_TPB, 0, stream>>>(d_in, d_out, in_off, comp_sz, out_off, orig_sz); break; \
            case 2: rareDecodeKernel<uint16_t, CS_VAL><<<n_chunks, RARE_TPB, 0, stream>>>(d_in, d_out, in_off, comp_sz, out_off, orig_sz); break; \
            case 4: rareDecodeKernel<uint32_t, CS_VAL><<<n_chunks, RARE_TPB, 0, stream>>>(d_in, d_out, in_off, comp_sz, out_off, orig_sz); break; \
            case 8: rareDecodeKernel<uint64_t, CS_VAL><<<n_chunks, RARE_TPB, 0, stream>>>(d_in, d_out, in_off, comp_sz, out_off, orig_sz); break; \
            default: throw std::runtime_error("RAREStage: word_size must be 1, 2, 4, or 8");                 \
        }                                                                                                     \
        break;
    switch (chunk_size) {
        FZ_RARE_DECODE_CASE(4096)
        FZ_RARE_DECODE_CASE(8192)
        FZ_RARE_DECODE_CASE(16384)
        default: throw std::runtime_error("RAREStage: chunk_size must be 4096, 8192, or 16384");
    }
#undef FZ_RARE_DECODE_CASE
}
...
void RAREStage::postStreamSync(cudaStream_t stream) {
    ...
    FZ_LOG(DEBUG, "RARE encode done: %.1f KB -> %.1f KB  ratio %.2fx",
           cached_orig_bytes_ / 1024.0f, actual_output_size_ / 1024.0f, ratio);
}

std::unordered_map<std::string, size_t>
RAREStage::getActualOutputSizesByName() const {
    if (tail_readback_pending_) {
        ...
        const size_t total_out = (size_t)tail_off + (size_t)tail_sz;
        const_cast<RAREStage*>(this)->actual_output_size_ = (total_out + 3) & ~size_t(3);
        if (tail_output_ptr_ && actual_output_size_ > total_out) {
            FZ_CUDA_CHECK(cudaMemsetAsync(tail_output_ptr_ + total_out, 0,
                                          actual_output_size_ - total_out, tail_readback_stream_));
            FZ_CUDA_CHECK(cudaStreamSynchronize(tail_readback_stream_));
        }
        const_cast<RAREStage*>(this)->tail_output_ptr_  = nullptr;
        tail_readback_pending_ = false;
        tail_readback_stream_  = nullptr;
    }
...
size_t RAREStage::getActualOutputSize(int index) const {
    if (index != 0) return 0;
    ...
    const std::vector<size_t>& sizes)
{
    if (inputs.empty() || outputs.empty() || sizes.empty())
        throw std::runtime_error("RAREStage: invalid inputs/outputs");
    ...
    if (chunk_size_ != 4096 && chunk_size_ != 8192 && chunk_size_ != 16384)
        throw std::runtime_error(
            "RAREStage: chunk_size must be 4096, 8192, or 16384; got "
            + std::to_string(chunk_size_));
    if (word_size_ != 1 && word_size_ != 2 && word_size_ != 4 && word_size_ != 8)
        throw std::runtime_error(
            "RAREStage: word_size must be 1, 2, 4, or 8; got "
    ...
    if (!is_inverse_) {
        ...
        if (n_chunks > scratch_capacity_) {
            ...
            if (pool) {
                d_scratch_    = (uint8_t*) pool->allocate(n_chunks * (size_t)chunk_size_, stream, "rare_scratch", true);
                d_sizes_dev_  = (uint32_t*)pool->allocate(n_chunks * sizeof(uint32_t),    stream, "rare_sizes",   true);
                d_clean_dev_  = (uint32_t*)pool->allocate(n_chunks * sizeof(uint32_t),    stream, "rare_clean",   true);
                d_dst_off_dev_= (uint32_t*)pool->allocate(n_chunks * sizeof(uint32_t),    stream, "rare_offsets", true);
                if (!d_scratch_ || !d_sizes_dev_ || !d_clean_dev_ || !d_dst_off_dev_)
                    throw std::runtime_error("RAREStage: failed to allocate persistent forward scratch from MemoryPool");
        ...
        FZ_LOG(TRACE, "RARE encode: %.1f KB in, %u chunks, word_size %d",
        ...
        rareStripFlagKernel<<<grid256, 256, 0, stream>>>(d_sizes_dev_, d_clean_dev_, n_chunks_u);

        // (4) Exclusive prefix sum of clean sizes → payload-relative offsets.
        {
            auto scan_tmp = fz::backend::withTempStorage(pool, stream, "rare_cub_scan_tmp",
        ...
        rareAddOffsetKernel<<<grid256, 256, 0, stream>>>(d_dst_off_dev_, n_chunks_u, (uint32_t)header_size);
        ...
        rarePackKernel<<<(int)n_chunks, 512, 0, stream>>>(
    ...
    // ── Inverse (decompress) ─────────────────────────────────────────────
        ...
        uint32_t* d_in_off  = alloc_u32("rare_inv_in_off");
        uint32_t* d_comp_sz = alloc_u32("rare_inv_comp_sz");
        uint32_t* d_out_off = alloc_u32("rare_inv_out_off");
        uint32_t* d_orig_sz = alloc_u32("rare_inv_orig_sz");
        ...
        FZ_LOG(DEBUG, "RARE decode done: %.1f KB -> %.1f KB",
