...
enum class StageType : uint16_t {
    ...
    RRE        = 26,   ///< Repeated-word bitmap reducer with recursive bitmap compression (LC component)
    RARE       = 27,   ///< Adaptive top-bit matching generalization of RRE (LC component)
    RAZE       = 28,   ///< Adaptive leading-zero-bit generalization of RZE (LC component)
    CLOG       = 29,   ///< Per-subchunk leading-zero compression and bit packing (LC framework component)
    HCLOG      = 30,   ///< CLOG bit packing with per-subchunk TCMS selection (LC framework component)
    ...
    // archive that used it should fail loudly, not silently collide.
    CDF97      = 39,   ///< CDF 9/7 biorthogonal wavelet transform (SPERR's DWT front-half)
...
inline std::string stageTypeToString(StageType type) {
    switch (type) {
        ...
        case StageType::GOLOMB_RICE: return "GolombRice";
        default:                     return "Unknown";
    }
}
